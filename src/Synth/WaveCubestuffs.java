...
public class WaveCubestuffs
{
    ...
    public static double interpolateWaveTable(double[][] waveTable,double x,double y)//x and y ∈ [0,1)
    {
        ...
        int yc=(int)r.ceil(y);
        int xc=(int)r.ceil(x);
        double x0=r.lerp(x,xf,xc,waveTable[xf%xl][yf%yl],waveTable[xc%xl][yf%yl]);
        double x1=r.lerp(x,xf,xc,waveTable[xf%xl][yc%yl],waveTable[xc%xl][yc%yl]);
        double xy=r.lerp(y,yf,yc,x0,x1);
